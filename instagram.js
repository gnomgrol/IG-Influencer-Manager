...
exports.getChatAll = function (session, chatId){
  return new Promise((resolve, reject) => {
    Client.Thread.getById(session, chatId).then((result) => {
        var tFeed = new Client.Feed.ThreadItems(session, chatId);
        var params = { delay: 100, every: 200, pause: 30000, maxErrors: 9, limit: 10000 };
        tFeed.all(params).then((r) => {
            result.items = r;
            resolve(result)
        })
    }).catch(reject)
