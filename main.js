  ...
  if (!mainWindow) {
    ...
    mainWindow.on('app-command', (e, cmd) => {
        ...
        if (cmd === 'browser-backward') {
            mainWindow.webContents.send("webviewBack");
        }
        if (cmd === 'browser-forward') {
            mainWindow.webContents.send("webviewForward");
        }
    })

  }
  mainWindow.setTitle('IG Manager')
...
let timeoutObj2;
function getFullChat (evt, id) {
  instagram.getChatAll(session, id).then((chat) => {
    mainWindow.webContents.send('chat', chat, true);
    if (timeoutObj2) clearTimeout(timeoutObj2)

    timeoutObj2 = setTimeout(getFullChat, 15000, {}, id);
  }).catch(() => { setTimeout(getFullChat, RATE_LIMIT_DELAY, evt, id)})
}
...
electron.ipcMain.on('getFullChat', getFullChat)
...
electron.ipcMain.on("newMessageArrived", (evt, data) => {
